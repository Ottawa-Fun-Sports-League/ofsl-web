-- Update get_users_paginated_admin to support dynamic sport filters
...
BEGIN
    ...
    IF array_length(p_sports_in_league, 1) IS NOT NULL AND array_length(p_sports_in_league, 1) > 0 THEN
        v_where_conditions := array_append(v_where_conditions,
            'EXISTS (
               SELECT 1 FROM league_payments lp
               JOIN leagues l ON l.id = lp.league_id
               WHERE lp.user_id = cd.profile_id AND COALESCE(lp.is_waitlisted,false) = false
...
COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users with dynamic sport filters.';
