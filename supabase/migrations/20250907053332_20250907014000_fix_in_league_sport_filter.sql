-- Refine dynamic sport 'in league' filter to include team memberships and individual registrations
...
BEGIN
    ...
    -- Dynamic sports: in active leagues for any selected sports
            ...
            '(
             ...
             OR EXISTS (
               ...
               WHERE lp.user_id = cd.profile_id
                 ...
                 AND COALESCE(lp.is_waitlisted,false) = false
                 AND (l2.active = true OR l2.end_date IS NULL OR l2.end_date >= CURRENT_DATE)
...
COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users with dynamic sport filters (team + individual).';;
