-- Fix ALL ambiguous auth_id column references in get_users_paginated_admin function
-- Error: column reference "auth_id" is ambiguous at line checking admin status
...
-- Recreate with all ambiguities fixed
...
BEGIN
    ...
    -- FIXED: Qualify the auth_id column with table name
    IF NOT EXISTS (
...
COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users. Fixed ALL auth_id ambiguities by qualifying table references and renaming conflicting columns.';