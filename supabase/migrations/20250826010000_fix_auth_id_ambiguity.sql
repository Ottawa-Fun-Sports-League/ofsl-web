-- Fix ambiguous auth_id column reference in get_users_paginated_admin function
-- Error: column reference "auth_id" is ambiguous
...
RETURNS TABLE(
    profile_id TEXT,  -- Fixed: Changed from UUID to TEXT to match users.id type
    auth_id UUID,
    name TEXT,
    email TEXT,
    phone TEXT,
...
BEGIN
    ...
    IF p_search != '' THEN
        v_where_conditions := array_append(v_where_conditions, 
            '(LOWER(combined_data.name) LIKE ''' || v_search_term || ''' OR LOWER(combined_data.email) LIKE ''' || v_search_term || ''')');
    END IF;
    
    IF p_administrator THEN
        v_where_conditions := array_append(v_where_conditions, 'combined_data.is_admin = true');
    END IF;
    
    IF p_facilitator THEN
        v_where_conditions := array_append(v_where_conditions, 'combined_data.is_facilitator = true');
    END IF;
    
    IF p_pending_users THEN
        v_where_conditions := array_append(v_where_conditions, 'combined_data.status IN (''unconfirmed'', ''confirmed_no_profile'', ''profile_incomplete'')');
    END IF;
    
    IF p_active_player THEN
        v_where_conditions := array_append(v_where_conditions, 'combined_data.status = ''active''');
    END IF;
    
    -- Sport-specific filters
    IF p_volleyball_players_in_league THEN
        v_where_conditions := array_append(v_where_conditions, 
            'EXISTS (SELECT 1 FROM league_payments lp JOIN leagues l ON lp.league_id = l.id JOIN sports s ON l.sport_id = s.id WHERE lp.user_id = combined_data.profile_id AND s.name = ''Volleyball'' AND lp.is_waitlisted = false)');
    END IF;
    
    IF p_badminton_players_in_league THEN
        v_where_conditions := array_append(v_where_conditions, 
            'EXISTS (SELECT 1 FROM league_payments lp JOIN leagues l ON lp.league_id = l.id JOIN sports s ON l.sport_id = s.id WHERE lp.user_id = combined_data.profile_id AND s.name = ''Badminton'' AND lp.is_waitlisted = false)');
    END IF;
    
    IF p_volleyball_players_all THEN
        v_where_conditions := array_append(v_where_conditions, 
            'combined_data.user_sports_skills::text LIKE ''%"sport":"Volleyball"%''');
    END IF;
    
    IF p_badminton_players_all THEN
        v_where_conditions := array_append(v_where_conditions, 
            'combined_data.user_sports_skills::text LIKE ''%"sport":"Badminton"%''');
    END IF;
    
    IF p_players_not_in_league THEN
        v_where_conditions := array_append(v_where_conditions, 
            'NOT EXISTS (SELECT 1 FROM league_payments lp WHERE lp.user_id = combined_data.profile_id AND lp.is_waitlisted = false)');
    END IF;

    -- Build sort clause
    CASE p_sort_field
        WHEN 'name' THEN v_sort_sql := 'combined_data.name';
        WHEN 'email' THEN v_sort_sql := 'combined_data.email';
        WHEN 'date_created' THEN v_sort_sql := 'combined_data.date_created';
        WHEN 'last_sign_in_at' THEN v_sort_sql := 'combined_data.last_sign_in_at';
        WHEN 'status' THEN v_sort_sql := 'combined_data.status';
        ELSE v_sort_sql := 'combined_data.date_created';
    END CASE;
    ...
    -- Fixed: Use COALESCE to handle the auth_id ambiguity
    v_final_query := '
    WITH auth_users AS (
        SELECT 
            au.id as auth_user_id,  -- Renamed to avoid ambiguity
    ...
    user_profiles AS (
        SELECT 
            u.id as profile_id,
            u.auth_id as profile_auth_id,  -- Renamed to avoid ambiguity
            u.name,
    ...
    combined_data AS (
        SELECT 
            up.profile_id,
            COALESCE(up.profile_auth_id, au.auth_user_id) as auth_id,  -- Fixed: use renamed columns
            up.name,
            COALESCE(up.profile_email, au.auth_email) as email,  -- Fixed: use renamed columns
            up.phone,
            up.is_admin,
            up.is_facilitator,
            ...
            au.last_sign_in_at
        FROM auth_users au
        LEFT JOIN user_profiles up ON au.auth_user_id = up.profile_auth_id  -- Fixed: use renamed columns
    ),
    payment_totals AS (
        SELECT 
            lp.user_id,
...
COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users with filtering and sorting. Fixed ambiguous auth_id column references by renaming columns in CTEs.';