-- Fix get_users_paginated_admin function return type mismatch
-- Issue: profile_id was defined as UUID but users.id is TEXT type

-- Drop the function first to change return types
...
-- Create or replace the function with correct return types
...
COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users with filtering and sorting capabilities. Fixed return type for profile_id from UUID to TEXT to match users.id column type.';