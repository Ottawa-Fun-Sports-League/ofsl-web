-- Extend get_users_paginated_admin with league, team, and tier filters
...
CREATE OR REPLACE FUNCTION get_users_paginated_admin(
    ...
    p_league_ids BIGINT[] DEFAULT '{}',
    p_team_ids BIGINT[] DEFAULT '{}',
    p_league_tier_filters TEXT[] DEFAULT '{}'
)
RETURNS TABLE(
    profile_id TEXT,
    auth_id UUID,
...
GRANT EXECUTE ON FUNCTION get_users_paginated_admin(
    INTEGER, INTEGER, TEXT, TEXT, TEXT,
    BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN,
    BOOLEAN, INT[], INT[], BIGINT[], BIGINT[], TEXT[]
) TO authenticated;

COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users with dynamic filters for roles, sports, leagues, teams, and tiers.';
