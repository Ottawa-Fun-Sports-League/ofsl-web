-- Broaden 'in league' sport filter to include all individual registrations (including waitlisted) to align with Manage Leagues counts
...
BEGIN
    ...
    -- Dynamic sports: in leagues (team or individual). For individual regs, include all registrations (active + waitlist) to match Manage Leagues counts.
    ...
    -- Dynamic sports: has skills for any selected sports (checks JSONB sport_id)
    ...
    SELECT profile_id, auth_id, name, email, phone, is_admin, is_facilitator, date_created, date_modified, team_ids, league_ids,
           user_sports_skills, status, confirmed_at, last_sign_in_at, auth_created_at, total_count, total_paid, total_owed, current_registrations
    FROM filtered_data';
...
COMMENT ON FUNCTION get_users_paginated_admin IS 'Admin function to retrieve paginated users with dynamic sport filters (team + individual, includes waitlist for individual).';;
