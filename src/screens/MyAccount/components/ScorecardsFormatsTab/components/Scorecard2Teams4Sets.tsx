...
interface Scorecard2Teams4SetsProps {
...
export function Scorecard2Teams4Sets({ teamNames, onSubmit, isTopTier = false, pointsTierOffset = 0, resultsLabel, tierNumber, initialSets, initialSpares, submitting = false }: Scorecard2Teams4SetsProps) {
      ...
      <div className="bg-[#F8F8F8] rounded-b-lg px-3 sm:px-4 py-3 space-y-3">
        ...
        <div className="mt-3 px-4 py-3 bg-red-50 border border-red-200 rounded-[10px] relative">
          {(() => {
            ...
            // Determine overall winner/loser (wins > losses, then differential)
            ...
            if (allEntered && sorted.length === 2) {
              ...
              role[sorted[1]] = 'loser';
              if (isTopTier) {
                // Top tier winner cannot move up; show explicit positions where helpful
                movement[sorted[0]] = 'Stay -> A';
                movement[sorted[1]] = 'Down';
              } else if (pointsTierOffset === 0) {
                // Bottom tier loser cannot move down
            ...
            // Points: Base 3 (loser) / 5 (winner) +2 per tier from bottom
            ...
            return (
              <div>
                ...
                <span className="absolute right-4 top-3 text-[11px] text-[#4B5563]">
                  <span className="font-semibold">Tier {tierDisplay}:</span> Base 3/5 Bonus +{tierBonus}
                </span>
