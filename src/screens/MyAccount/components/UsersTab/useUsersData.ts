...
export function useUsersData() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      // Debug variables for development/debugging
      void processedUsers.filter(u => u.current_registrations && u.current_registrations.length > 0); // _usersWithRegistrations
      void processedUsers.filter(u => u.profile_id); // _usersWithProfile
      void processedUsers.filter(u => !u.profile_id); // _usersWithoutProfile
      void processedUsers.filter(u => u.league_ids && u.league_ids.length > 0); // _usersWithLeagueIds
      void processedUsers.filter(u => 
