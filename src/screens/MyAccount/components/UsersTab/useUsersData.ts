...
import { User, UserFilters, SortField, SortDirection, PaginationState } from './types';
...
export function useUsersData() {
  ...
  }, [debouncedSearchTerm, filters, sortField, sortDirection, pagination.currentPage, pagination.pageSize]);
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      const processedUsers: User[] = paginatedData.map((row: any) => {
  ...
  }, [userProfile?.id, pagination.currentPage, pagination.pageSize, debouncedSearchTerm, sortField, sortDirection, filters, showToast]);
