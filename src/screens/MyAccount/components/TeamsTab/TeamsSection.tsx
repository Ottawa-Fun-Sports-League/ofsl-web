import { User, MapPin, Trash2, UserPlus, Clock, DollarSign, Users } from 'lucide-react';
      ...
      {teams.length === 0 ? (
        ...
        <div className="space-y-4">
          {teams.map(team => {
            ...
            const leagueFee = team.league?.cost || teamPayment?.league_cost || teamPayment?.amount_due || 0;
            
            return (
              <div key={team.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200">
                <div className="mb-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4">
                    <div className="flex-1">
                      <h4 className="font-semibold text-lg text-[#6F6F6F] mb-2">{team.name}</h4>
                      <div className="mb-3">
                        {team.league?.id ? (
                          <div>
                            <Link 
                          ...
                          <p className="text-sm text-[#B20000] font-medium">{team.league?.name || 'Unknown League'}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-3 sm:mt-0 sm:ml-4">
                      ...
                      <span className={`px-3 py-1 text-xs rounded-full whitespace-nowrap font-medium ${
                        isCaptain ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'
                      }`}>
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-3 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#6F6F6F]">Location: {team.league?.location || 'TBD'}</span>
                    </div>
                    
                    <div className="flex items-center gap-2">
                      <User className="h-5 w-5 text-blue-500 flex-shrink-0" />
                      <span className="text-[#6F6F6F]">Team Size: {team.roster.length} players</span>
                    </div>

                    {isCaptain && teamPayment?.due_date && (
                      <div className="flex items-center gap-2">
                        <Clock className="h-5 w-5 text-orange-500 flex-shrink-0" />
                        <span className="text-[#6F6F6F]">Due: {new Date(teamPayment.due_date).toLocaleDateString()}</span>
                      </div>
                    )}
                    ...
                    {isCaptain && (teamPayment || leagueFee > 0) && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[#6F6F6F]">${(teamPayment?.amount_paid || 0).toFixed(2)} / ${(leagueFee * 1.13).toFixed(2)} (${leagueFee.toFixed(2)} + HST)</span>
                      </div>
                    )}
                  </div>
                </div>
                      ...
                      >
                        {isCaptain ? (
                          ...
                          <>
                            ...
                            <span className="hidden sm:inline">View Teammates</span>
